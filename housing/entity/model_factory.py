...
def evaluate_regression_model(model_list:list,x_train:np.ndarray,y_train:np.ndarray,x_test:np.ndarray,y_test:np.ndarray,base_accuracy:float=0.6)->MetricInfoArtifact:
    ...
    try:
        ...
        for model in model_list:
            ...
            #if model accuracy is greater than the base accuracy and train and test score are within certain threshold
...
class ModelFactory:
    ...
    def update_property_of_class(instance_ref:object,property_data:dict):
        try:
            if not isinstance(property_data,dict):
                raise Exception("property_data parameter is required in dictionary")
            print(property_data)
    ...
    def class_for_name(module_name:str,class_name:str):
        try:
            #load the module wil raise error if module can not be loaded
            module=importlib.import_module(module_name)
            #get the class, will raise Attribute error if class can not be found
